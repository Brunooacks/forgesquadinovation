...
Type `/forgesquad` to start. The framework will guide you through:
1. Creating a squad for your project
2. Configuring agents and pipeline
3. Executing with checkpoints and approvals
4. Generating production-ready output with full audit trail

Every step requires your approval. Every artifact is tracked.

### Commands

- `/forgesquad` — Main menu
- `/forgesquad create` — Create a new engineering squad (Architect guides you)
- `/forgesquad run <name>` — Execute a squad pipeline with full audit trail
- `/forgesquad run <name> --resume <run-id>` — Resume pipeline from last checkpoint
- `/forgesquad list` — List available squads
- `/forgesquad backup` — Create backup snapshot
- `/forgesquad report <name>` — Generate status report for project manager
- `/forgesquad help` — See all commands

### How Execution Works

When you run `/forgesquad run`, the runner:
1. Validates all squad files (squad.yaml, pipeline.yaml, agent files)
2. Creates a run directory with unique ID for all artifacts
3. Initializes state.json and audit-trail.json
4. Executes each pipeline step in order
5. **Pauses at every checkpoint** for your [S]im/[N]ao/[R]evisar decision
6. **Pauses at every agent handoff** for your approval
7. Generates artifacts at every step with SHA-256 hashes
8. Produces a final report with full metrics
